...
# Example usage:
#
#     bash scripts/submit_url_signals.sh http://url1 http://url2 http://url3
#
# If you are not running on the default port, remember to override the port here
# as well:
#
#     PORT=1234 bash scripts/submit_url_signals.sh http://url1 http://url2 http://url3

for url in "$@"
do
    echo "Submitting URL: $url"
    response=$(echo '{"content": {"value": "'$url'", "type": "URL"}, "source": {"name": "USER_REPORT", "author": "'$USER'", "create_time": "'$(date -u +"%Y-%m-%dT%H:%M:%S")'"}}' | \
